...
interface LocationInputType extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type'> {
  ...
  ({ label, icon, className = '', ...inputProps }, ref) => {
    return (
      <div className={clsx('relative', className)}>
        <div className='absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none sm:pl-6'>
          {icon}
        </div>
        <input
          ref={ref}
